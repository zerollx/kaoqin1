    ...
    layui.use(['laydate', 'laypage', 'layer', 'table', 'carousel', 'upload', 'element', 'slider', 'util'], function () {
        ...
        table.render({
            ...
            , cols: [[ //表头
                ...
                , {field: 'departCode', title: '部门编号', width: 130, sort: true, totalRow: true}
                , {field: 'departName', title: '部门名', width:200, sort: true}
                , {field: 'departLeading', title: '部门负责人', width: 200, sort: true}
                , {field: 'departDuty', title: '部门职责', width: 160, sort: true}
                , {field: 'departSuperDepart', title: '上级部门', width: 150, sort: true}
        ...
        table.on('toolbar(test)', function (obj) {
            ...
            switch (obj.event) {
                case 'add':
                    layer.msg('添加');
                    xadmin.open("添加页面", "Department_Add.jsp", 1000, 650);
                    break;
                case 'update':
                    if (data.length === 0) {
                        layer.msg('请选择一行');
        ...
        table.on('tool(test)', function (obj) { //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'detail') {
                ...
                xadmin.open('修改界面', 'findById?id=' + obj.data.id, 1200, 660)
            }
        });
