    ...
    layui.use(['laydate', 'laypage', 'layer', 'table', 'carousel', 'upload', 'element', 'slider', 'util'], function () {
        ...
        table.render({
            ...
            , cols: [[ //表头
                ...
                , {field: 'empCode', title: '员工编码', width: 100, sort: true, totalRow: true}
                , {field: 'empName', title: '姓名', width: 150, sort: true}
                , {field: 'empSex', title: '性别', width: 100, sort: true}
                , {field: 'empAge', title: '年龄', width: 100, sort: true}
                , {field: 'empNation', title: '名族', width: 150, sort: true}
                , {templet: '<div>{{d.station.sname}}</div>', title: '岗位', width: 130,}
                , {field: 'empSalary', title: '薪水', width: 130, sort: true}
        ...
        table.on('toolbar(test)', function (obj) {
            ...
            switch (obj.event) {
                case 'add':
                    layer.msg('添加');
                    xadmin.open("添加页面", "/allPositionId", 1000, 650);
                    break;
                case 'update':
                    if (data.length === 0) {
                        layer.msg('请选择一行');
        ...
        table.on('tool(test)', function (obj) { //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'detail') {
                ...
                xadmin.open('修改界面', 'updateById?id=' + obj.data.id, 1200, 660)
            }
        });
