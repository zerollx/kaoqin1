    ...
    layui.use(['laydate', 'laypage', 'layer', 'table', 'carousel', 'upload', 'element', 'slider', 'util'], function () {
        ...
        table.render({
            ...
            , cols: [[ //表头
                ...
                , {field: 'leavePeople', title: '请假人', width: 120, sort: true, totalRow: true}
                , {field: 'start', title: '开始时间', width: 200, sort: true}
                , {field: 'end', title: '结束时间', width: 200, sort: true}
                , {field: 'reason', title: '请假原因', width: 130, sort: true}
                , {
                    field: 'status', title: '状态', width: 130, sort: true, templet: function (d) {
                        if (d.status == 0) {
                            return '等待审核'
                        } else if(d.status == 1) {
                            return '审核通过'
...
// , {title: '分类名', width: 150, templet: '<div>{{d.productCategory.categoryName}}</div>'}
        ...
        table.on('toolbar(test)', function (obj) {
            ...
            switch (obj.event) {
                case 'add':
                    layer.msg('添加');
                    xadmin.open("添加页面", "Leave_Add.jsp", 1000, 650);
                    break;
                case 'update':
                    if (data.length === 0) {
                        layer.msg('请选择一行');
        ...
        table.on('tool(test)', function (obj) { //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'detail') {
                ...
                xadmin.open('修改界面', 'LeaveFindById?id=' + obj.data.id, 1200, 660)
            }
        });
